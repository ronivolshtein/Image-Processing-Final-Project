...
def main():
    ...
    for task in tasks:
        ...
        # We use lineplot which automatically calculates the mean and confidence interval for each level
        ...
        # Filter out 'clean' images or infinite SNR values for a cleaner scatter/line plot
        snr_df = task_df.replace([float('inf'), -float('inf')], pd.NA).dropna(subset=['snr_distorted_db'])
        
        # Use scatter plot with a trendline (regplot/lmplot) or simple lineplot
        ...
        # Invert X axis so that the graph moves from "Clean/High SNR" on the left to "Noisy/Low SNR" on the right
